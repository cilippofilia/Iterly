...
- Align `DashboardViewModel.upcomingTasks(from:)` with the same comparator (it currently sorts due-date-first and excludes tasks without due dates — fine for "Upcoming", but it should reuse the shared logic where possible).
- Add unit tests for the comparator (see §6).

### 1.2 Hide closed projects on the Dashboard screen — **S**
Both `@Query` predicates in `Iterly/Tabs/Dashboard/DashboardView.swift` filter only on `isPinned`, so closed projects still show up in Pinned and Projects sections on Dashboard. Add `status != .closed` to both predicates (use the raw-value comparison if `#Predicate` can't compare the enum directly — the model stores it via a raw value). The Projects tab already handles this correctly via `ProjectsViewModel.splitProjects()`; Dashboard should match. Also decide whether a *pinned* project that gets closed should be auto-unpinned (recommended: yes, on status change to `.closed`).

### 1.3 Fix Activity Overview header layout (Dashboard + Activity) — **S**
...
- In the **Dashboard** branch the inner `VStack` has no `alignment: .leading`, so "Activity Overview" and the legend center-align instead of leading-align.
...
Fix: give the Dashboard `VStack` `alignment: .leading`; for the Activity branch use a plain `HStack` with a `Spacer()` (or `.center` alignment) between title and legend rather than baseline + infinite-width frame. Also fix the `#Preview("Dashboard Style")` which passes `isDashboardView: false` — it previews the wrong layout, which is how this regression slipped through.
...
**Decision (confirmed):** a rotating "Tip of the day" card on **Dashboard**, dismissible, with a browse-all list.

- Static tip catalog bundled in the app (a `ProductivityTip` struct + array, or a JSON resource): short, app-aware tips ("Pin up to 4 projects to keep them on Dashboard", "Set due dates so tasks appear in Upcoming", "Log activity daily to build your streak"…).
- `TipCardView` on Dashboard (likely in `DashboardAvailableView` near the top): rotates by day-of-year, "dismiss for today" persisted via `@AppStorage`.
...
9. **Empty states & onboarding — S/M.** First-run experience explaining the Dashboard/Projects/Activity flow; richer empty states with calls to action instead of blank sections.
...
- **Unit tests (Swift Testing) — M.** Start with pure logic: task comparator (§1.1), `TaskSectionsBuilder`, `ProjectsViewModel.splitProjects`, `DashboardViewModel.upcomingTasks`, `TaskOverdueCalculator`, App Store ID extraction in `ProjectRelease`. None of it needs a UI.
